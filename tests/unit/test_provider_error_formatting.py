...
def test_parse_upstream_error_message():
    raw = (
...
def test_disabled_key_embedding_dump_is_credential_error():
...
async def test_resolve_chat_stream_error_message_async_probes_llm_on_generic(monkeypatch):
    ...
    assert "gpt-4o-mini" in await resolve_chat_stream_error_message_async(
        "An unknown error occurred."
    )
...
async def test_probe_checks_non_selected_providers_with_keys(monkeypatch):
    """Langflow receives every provider key, so a revoked key must still surface when
    ...
    class FakeConfig:
        ...
        class providers:
            ...
            ollama = FakeProvider(endpoint="http://ollama.internal:11434")
            custom = FakeProvider()
