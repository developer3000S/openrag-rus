...
:::info Configure via `.env` to skip the wizard
If all required configuration is already provided through environment variables (such as `OPENAI_API_KEY`, `OLLAMA_*`, `OMNIROUTE_*`, `EMBEDDING_PROVIDER`, and `EMBEDDING_MODEL`), the onboarding wizard is automatically skipped and the application loads the configuration directly from the environment. See [Configuration](/reference/configuration) for the full list of supported environment variables.
:::

You can use the same provider or different providers for your language model and embedding model.
Additionally, you can select multiple embedding models.

OpenRAG also supports the [**OMNIROUTE**](/reference/configuration#model-provider-settings) provider, an OpenAI-compatible proxy for language models.
It is not listed in the application onboarding dialog; configure it later on the OpenRAG **Settings** page or with the `OMNIROUTE_*` environment variables.
...
6. Click **Complete**.
   ...
   After you configure the embedding model, OpenRAG uses your credentials and models to ingest some [initial documents](/knowledge-browse#default-documents). This tests the connection, and it allows you to ask OpenRAG about itself in the [**Chat**](/chat).
...
<TabItem value="OpenAI" label="OpenAI (default)" default>
