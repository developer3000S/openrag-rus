...
|----------|---------|-------------|
| `EMBEDDING_PROVIDER` | `openai` | Embedding model provider, as one of `openai` or `ollama`. |
...
| `LLM_PROVIDER` | `openai` | Language model provider, as one of `openai`, `ollama`, or `omniroute`. |
...
| `OMNIROUTE_API_KEY` | Not set | API key for the OMNIROUTE model provider, an OpenAI-compatible proxy. |
| `OMNIROUTE_HOST` | Not set | Base URL of the OMNIROUTE endpoint, for example `http://<host>/api/v1`. |
| `OMNIROUTE_MODEL` | `free-stack` | Default language model served by the OMNIROUTE endpoint. |

:::info OMNIROUTE provider
OMNIROUTE is an OpenAI-compatible proxy that you host or subscribe to.
It is available as a language model provider only (not as an embedding provider), and it can be configured in the OpenRAG **Settings** page or with the `OMNIROUTE_*` environment variables.
:::
...
| `LANGFLOW_SUPERUSER` | `langflow` | Username for the Langflow administrator user. If this variable isn't set, OpenRAG uses `langflow`. |
...
| `OPENRAG_BACKEND_PORT` | `8000` | Host port for the OpenRAG backend. The base `docker-compose.yml` doesn't publish the backend port because the frontend reaches it over the Docker Compose network. Publish it directly with the `docker-compose.backend-port.yml` override (`docker compose -f docker-compose.yml -f docker-compose.backend-port.yml up -d`), and change `OPENRAG_BACKEND_PORT` if port `8000` is already in use. This variable also configures the backend bind port for local `make backend` development runs. |
...
| `NEXT_ALLOWED_DEV_ORIGINS` | `http://localhost:3000` | Comma-separated list of hostnames (or IP:port) that are allowed to make requests to the Next.js development server's built-in proxy. The default `http://localhost:3000` only allows access from the same host. Add your LAN hostname or `http://<IP>:3000` to access OpenRAG from other machines on your network, otherwise every `/api/*` call returns `500` with `ECONNREFUSED` in the browser console. Also required when [running OpenRAG in development mode](/support/contribute). |
